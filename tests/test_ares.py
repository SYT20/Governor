"""Ares is tested INDEPENDENTLY of the Governor, and proven identical to the
frozen executor.
...
from governor.ares.executor import (
    Ares, AresLoop, BudgetExceeded, UnknownAction,
)
...
def test_ares_does_not_know_what_a_governor_is():
    ...
    """
    import ast
    import governor.ares.executor as mod
...
def test_unknown_action_raises(p4):
    with pytest.raises(UnknownAction):
        Ares(p4).execute("THINK_HARDER", p4.reset(0), p4.budget)
...
def test_loop_raises_rather_than_silently_skipping_an_action(p4):
    ...
    with pytest.raises(BudgetExceeded):
        AresLoop(p4).run(always_deep, 0, 2000.0)
